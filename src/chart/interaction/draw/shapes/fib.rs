...
use crate::drawings::shape::fib as fib_shape;

impl Chart {
    pub fn add_fib_retracement_at(&mut self, x_pixels: f32, y_pixels: f32) {
        let Some((world_x, price, price_span)) = self.drawing_anchor_at(x_pixels, y_pixels) else {
            return;
        };

        let cmd = fib_shape::add_command_from_anchor(world_x, price, price_span);
        let _ = execute_command(&mut self.drawings, cmd);
    }

    pub fn add_fib_retracement_from_pixels(
        &mut self,
        x1_pixels: f32,
        y1_pixels: f32,
        x2_pixels: f32,
        ...
        let cmd = fib_shape::add_command_from_points(start_index, start_price, end_index, end_price);
        let _ = execute_command(&mut self.drawings, cmd);
    }
}
